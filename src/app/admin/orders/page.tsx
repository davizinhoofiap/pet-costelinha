...
export default function AdminOrdersPage() {
  ...
  // Estado para Modal de Confirmação de Exclusão Individual (Ícone de Lixeira)
  ...
  const fetchOrders = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setOrders(data);
        setSelectedOrderIds([]); // Limpar seleção ao atualizar
      }
    } catch (err) {
  ...
  // Confirmar Exclusão em Massa dos Pedidos Selecionados
  const handleConfirmBulkDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast(`${selectedOrderIds.length} pedido(s) excluído(s) do banco de dados!`);
        setSelectedOrderIds([]);
        setIsBulkDeleteModalOpen(false);
      } else {
        showToast('Erro ao excluir os pedidos selecionados.');
      }
    } catch (err) {
      console.error(err);
  ...
  // Abrir Modal de Exclusão Individual (Lixeira)
  ...
  // Confirmar Exclusão Individual no Banco de Dados
  ...
  return (
    <AdminLayout>
      <div className="w-full space-y-6 font-sans relative">
        ...
        {/* BARRAS DE AÇÃO EM MASSA (EXCLUSÃO EM LOTE) */}
        {selectedOrderIds.length > 0 && (
          <div className="bg-slate-900 text-white p-4 rounded-2xl shadow-lg border border-slate-800 flex flex-col sm:flex-row justify-between items-center gap-3">
            <div className="flex items-center gap-3">
              <div className="w-7 h-7 rounded-full bg-rose-500 text-white font-black text-xs flex items-center justify-center shadow-xs">
            ...
            <div className="flex items-center gap-2.5 w-full sm:w-auto">
              ...
              <button
                ...
                className="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-extrabold rounded-xl flex items-center gap-1.5 transition-all shadow-xs cursor-pointer"
              >
        ...
        {/* CONTAINER DA TABELA DE PEDIDOS COM SELEÇÃO EM LOTE */}
          ...
          <div className="overflow-x-auto w-full max-w-full">
            <table className="w-full text-left text-xs border-collapse table-auto">
              <thead className="bg-slate-50 text-slate-500 uppercase tracking-wider font-bold border-b border-slate-200/80">
                <tr>
                  {/* Checkbox de Seleção Geral (Master Checkbox) */}
            ...
            <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden border border-slate-200 p-6 space-y-4">
              ...
              <div className="text-center space-y-1.5">
                <h3 className="text-base font-extrabold text-slate-900">
                  Excluir {selectedOrderIds.length} {selectedOrderIds.length === 1 ? 'Pedido Selecionado' : 'Pedidos Selecionados'}?
                </h3>
                <p className="text-xs text-slate-600 leading-relaxed">
                  Tem certeza que deseja apagar permanentemente <strong>{selectedOrderIds.length} {selectedOrderIds.length === 1 ? 'pedido selecionado' : 'pedidos selecionados'}</strong> do banco de dados? Esta ação não pode ser desfeita.
                </p>
              </div>
        ...
        {/* MODAL 2: CONFIRMAÇÃO DE EXCLUSÃO INDIVIDUAL (LIXEIRA) */}
